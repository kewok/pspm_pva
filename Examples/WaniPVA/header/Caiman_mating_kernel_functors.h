#ifndef CAIMAN_MATING_KERNEL_FUNCTORS_H
#define CAIMAN_MATING_KERNEL_FUNCTORS_H
...
struct female_fecundity_functor
{
	/* 
		...
		2: Egg survivorship
		3: the individual's final fecundity score
	*/ 
	...
	*/ 
	...
	void operator()(tuple t) {
		if (thrust::get<0>(t) == 1) /* if the individual is indeed a reproducing female */
			...
			if (number_of_available_nests[individuals_subpopulation] < number_of_reproductive_females[individuals_subpopulation])
				{
				float nesting_probability = number_of_available_nests[individuals_subpopulation]/(number_of_reproductive_females[individuals_subpopulation]); // This merely describes the probability that any given female will obtain a nesting a site. I do not explicitly assign females to nesting sites, to allow for random variation like the sudden availability of fortuitous sites, or sites suddenly becming uninhabitable, etc...
...
/*			int result = thrust::get<0>(t);*/
/*			float rand = thrust::get<2>(t);*/
/*			printf("Nests available %f Nest prob %f rand %f %d\n", number_of_available_nests[individuals_subpopulation], number_of_reproductive_females[individuals_subpopulation], rand, result );*/
